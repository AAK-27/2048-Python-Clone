...
async def newGame():
    global SCORE
    await generateNewBoard(0)
    SCORE = 0
    saveGame()

async def checkForGameOver():
    s = 0
    for move in (UP, DOWN, LEFT, RIGHT):
        board = copy.deepcopy(TILES)
        await slideTiles(board, move, False)
        if board == TILES:
            s += 1
    if s == 4:
...
async def addTile(TILES, z):
    ...
    if emptyTiles != ([] or None):
        if (emptyTiles.__len__() != 0):
            ...
            await newTileAnimation(newTiles)
            drawScreen(TILES)
            pygame.display.update()
            FPSCLOCK.tick(FPS)
            await asyncio.sleep(0)
...
async def generateNewBoard(z):
    ...
    await addTile(TILES, 2)

async def getSavedBoard():
...
async def makeMove(TILES, slideTo):
    board = copy.deepcopy(TILES)
    await slideTiles(TILES, slideTo, True)
    if board != TILES:
...
async def slideTiles(TILES, slideTo, updateBoard):
    global GAMEWON, SCORE, HIGHSCORE
    slides = []
    scores = 0
    board = copy.deepcopy(TILES)
    ...
    if slideTo in (UP, DOWN):
        ...
        for y in range(BOARDHEIGHT):
            ...
            for x in range(BOARDWIDTH):
                ...
                TILES[x] = column
    if updateBoard:
        await slideAnimation(slides, board)
        if (scores > 0):
            SCORE += scores
            if SCORE > HIGHSCORE:
                HIGHSCORE = SCORE
...
async def newTileAnimation(newTiles):
...
async def slideAnimation(slides, board):
    combineTiles = []
    for slide in slides:
    ...
    for f in range(1, animation_time, ANIMATIONSPEED):
        ...
        FPSCLOCK.tick(FPS)
        await asyncio.sleep(0)
    await combineAnimation(combineTiles)

async def combineAnimation(tiles):
