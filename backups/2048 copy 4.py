...
myappid = '2048v0.8' # arbitrary string
...
def main():
    global FPSCLOCK, DISPLAYSURF, TILES, SCORE, HIGHSCORE, SMALLFONT, MEDIUMFONT, LARGEFONT, GAMEOVER
    pygame.init()
...
def saveGame():
    ...
    for x in range(BOARDWIDTH):
        ...
        for y in range(BOARDHEIGHT):
            ...
            column[y] = tileVal
        TILES[x] = column
    scores = [str(SCORE), str(HIGHSCORE), 'yeet', 'y does this work this way???']
    TILES.append(scores)
    board = np.array(TILES)
    save = str(board)
