...
myappid = '2048v0.9.2' # Arbitrary string
...
def main():
    global FPSCLOCK, DISPLAYSURF, TILES, SCORE, HIGHSCORE, SMALLFONT, MEDIUMFONT, LARGEFONT, GAMEOVER, TITLEFONT, LARGETILEFONT, MEDIUMTILEFONT, SMALLTILEFONT, TINYTILEFONT, RESET_BUTTON
    pygame.init()
...
def addTile(TILES):
    ...
    if emptyTiles != ([] or None ):
        if (emptyTiles.__len__() != 0):
            ...
            newTileAnimation(newTileX, newTileY, newTileVal)
            drawScreen(TILES)
            pygame.display.update()
            FPSCLOCK.tick(FPS)
...
def newTileAnimation(newTileX, newTileY, newTileVal):
    animation_time = int(ANIMATIONTIME * 0.015)
    for i in range(0, animation_time, ANIMATIONSPEED):
        checkForQuit()
        tileX = newTileX
        tileY = newTileY
        tileColor, tfs, _ = getTileStyles(newTileVal)
        fontSize = i / animation_time * tfs
        ...
        textRect = text.get_rect(center=(tileX * (TILESIZE + GAPSIZE) + XMARGIN + PADDING + GAPSIZE, tileY * (TILESIZE + GAPSIZE) + TOPYMARGIN + PADDING + GAPSIZE))
        xPad = PADDING - (PADDING * i / animation_time)
        yPad = PADDING - (PADDING * i / animation_time)
        pygame.draw.rect(DISPLAYSURF, tileColor, (tileX * (TILESIZE + GAPSIZE) + XMARGIN + xPad + GAPSIZE, tileY * (TILESIZE + GAPSIZE) + TOPYMARGIN + yPad + GAPSIZE, i / animation_time * TILESIZE, i / animation_time * TILESIZE), 0, 3)
        DISPLAYSURF.blit(text, textRect)
        checkForQuit()
