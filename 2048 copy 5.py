...
def main():
    global FPSCLOCK, DISPLAYSURF, TILES, SCORE, HIGHSCORE, SMALLFONT, MEDIUMFONT, LARGEFONT, GAMEOVER, TITLEFONT, LARGETILEFONT, MEDIUMTILEFONT, SMALLTILEFONT, TINYTILEFONT
    pygame.init()
...
def slideAnimation(array, board):
    for f in range(1, ANIMATIONTIME, ANIMATIONSPEED):
        drawScreen(board)
        for t in range(len(array)):
            ...
            if startX == endX:
                x = startX
                y = startY - (f * (startY - endY) / ANIMATIONTIME)
            elif startY == endY:
                x = startX - (f * (startX - endX) / ANIMATIONTIME)
                y = startY
            if val == 0:
                value = ''
            else:
...
def gameLost():
    gameOver = True
    for s in range(0, ANIMATIONTIME, ANIMATIONSPEED):
        drawScreen(TILES)
        greyTint = s / ANIMATIONTIME * 186.15
...
def gameWon():
    gameWon = True
    for s in range(0, ANIMATIONTIME, ANIMATIONSPEED): # Fade into a Yellow Tint
        drawScreen(TILES)
        yellowTint = s / ANIMATIONTIME * 186.15
    ...
    while gameWon:
        ...
        DISPLAYSURF.blit(tryAgainSurf, tryAgainRect)
        # You Win! Text
        gameOverSurf = gameOverFont.render('You Win!', True, LIGHTTEXTCOLOR, None)
        gameOverRect = gameOverSurf.get_rect(center=(WINDOWWIDTH / 2, TOPYMARGIN + (TILESIZE + GAPSIZE) * (BOARDHEIGHT / 2) - 30))
        DISPLAYSURF.blit(gameOverSurf, gameOverRect)
